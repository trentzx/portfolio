...
function App() {
 ...
 const initialIntro = useRef((() => {try{return !sessionStorage.getItem('signal-intro');}catch{return true;}})());
 ...
 const screen = <Screen route={route} go={navigate} sound={sound} toggleSound={() => setSound(v=>!v)} active={entered && power !== 'waking'}/>;
 const flatTV = <div className={`flat-tv ${entered ? 'flat-entered' : ''}`}><div className="flat-bezel">{screen}</div><div className="flat-hardware"><div className="flat-speaker"/><span>signal</span><i/><b/></div></div>;
 ...
 {flat ? (entered ? flatTV : flatConsole) : <SceneBoundary fallback={entered ? flatTV : flatConsole}><Suspense fallback={<div className="scene-loading"><Tv/>Tuning in…<button onClick={()=>setLite(true)}>Open lightweight view</button></div>}><div className="scene-transition" key={entered ? "crt" : "console"}>{entered ? <Television entered={entered} motion={motion} reduced={reduced} onFailure={()=>setWebgl(false)}>{screen}</Television> : <Console motion={motion} reduced={reduced} mobile={mobile} powering={power === 'charging'} onFailure={()=>setWebgl(false)}/>}</div></Suspense></SceneBoundary>}
