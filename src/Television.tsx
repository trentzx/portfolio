...
function Model({ children, entered, motion, reduced }: {children: ReactNode; entered: boolean; motion: MutableRefObject<MotionState>; reduced:boolean}) {
 ...
 useFrame((_, delta) => {
  ...
  const targetZ = entered ? fit - .95 : fit + .7;
 ...
 return <><group ref={group}>
  ...
  <Html transform rotation={[0,Math.PI,0]} position={[0,-.88,-2.005]} distanceFactor={.75} style={{pointerEvents:'none'}}><div className="rear-label">SIGNAL STUDIO • MODEL 006<br/>VIDEO IN · L · R · AC IN</div></Html>
 ...
 </group><ContactShadows position={[0,-2.7,0]} opacity={.25} scale={16} blur={2.8} far={5} resolution={256} frames={1}/></>;
}
export default function Television({children, entered, motion, reduced, onFailure}: {children:ReactNode;entered:boolean;motion:MutableRefObject<MotionState>;reduced:boolean;onFailure:()=>void}) {
 return <Canvas frameloop="demand" dpr={[1,1.6]} camera={{position:[0,0,11.5],fov:36}} gl={{antialias:true,alpha:true,powerPreference:'low-power'}} fallback={<div>3D unavailable. Use the accessible channel view.</div>}><ContextGuard onFailure={onFailure}/><ambientLight intensity={1.5}/><hemisphereLight args={['#ffffff','#727b6e',1.1]}/><directionalLight position={[-5,8,6]} intensity={3.2} color="#fff6e5"/><directionalLight position={[6,2,-3]} intensity={2} color="#d3e6f0"/><Suspense fallback={null}><Model entered={entered} motion={motion} reduced={reduced}>{children}</Model></Suspense></Canvas>;
}
